...
impl<DT, ET> WorkerRegistry<DT, ET> {
    ...
    {
        fn factory<T: Worker<Data = DT, Error = ET> + serde::de::DeserializeOwned + 'static, DT, ET>(
    ...
    pub fn build(&self, name: &str, json: serde_json::Value) -> Result<BoxedJob<DT, ET>, OxanusError> {
        let factory = self
            .jobs
            .get(name)
            .ok_or_else(|| OxanusError::GenericError(format!("Job type {} not registered", name)))?;
        match factory(json) {
            Ok(job) => Ok(job),
